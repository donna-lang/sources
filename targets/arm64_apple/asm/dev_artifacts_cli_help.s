.data
.balign 8
_str0:
	.ascii "donna"
	.byte 0
/* end data */

.data
.balign 8
_str2:
	.ascii " "
	.byte 0
/* end data */

.data
.balign 8
_str5:
	.ascii "Usage:"
	.byte 0
/* end data */

.data
.balign 8
_str7:
	.ascii " donna "
	.byte 0
/* end data */

.data
.balign 8
_str9:
	.ascii "<COMMAND>"
	.byte 0
/* end data */

.data
.balign 8
_str12:
	.ascii "Commands:"
	.byte 0
/* end data */

.data
.balign 8
_str14:
	.byte 10
	.byte 0
/* end data */

.data
.balign 8
_str18:
	.ascii "Options:"
	.byte 0
/* end data */

.data
.balign 8
_str20:
	.byte 10
	.byte 0
/* end data */

.data
.balign 8
_donna_nil:
	.quad 0
/* end data */

.data
.balign 8
_str24:
	.ascii ""
	.byte 0
/* end data */

.data
.balign 8
_str31:
	.ascii ""
	.byte 0
/* end data */

.data
.balign 8
_str38:
	.ascii ""
	.byte 0
/* end data */

.data
.balign 8
_str45:
	.ascii ""
	.byte 0
/* end data */

.data
.balign 8
_str52:
	.byte 10
	.byte 0
/* end data */

.data
.balign 8
_str54:
	.ascii "donna "
	.byte 0
/* end data */

.data
.balign 8
_str56:
	.ascii " "
	.byte 226
	.byte 128
	.byte 148
	.ascii " "
	.byte 0
/* end data */

.data
.balign 8
_str59:
	.ascii "Usage:"
	.byte 0
/* end data */

.data
.balign 8
_str61:
	.ascii " "
	.byte 0
/* end data */

.data
.balign 8
_str70:
	.ascii ""
	.byte 0
/* end data */

.data
.balign 8
_str71:
	.byte 10
	.byte 0
/* end data */

.data
.balign 8
_str72:
	.ascii "Options:"
	.byte 0
/* end data */

.data
.balign 8
_str75:
	.byte 10
	.byte 0
/* end data */

.data
.balign 8
_str78:
	.byte 10
	.byte 0
/* end data */

.data
.balign 8
_str82:
	.ascii ""
	.byte 0
/* end data */

.data
.balign 8
_str92:
	.ascii ""
	.byte 0
/* end data */

.data
.balign 8
_str99:
	.byte 10
	.byte 0
/* end data */

.data
.balign 8
_str101:
	.ascii "build"
	.byte 0
/* end data */

.data
.balign 8
_str102:
	.ascii "Build the project"
	.byte 0
/* end data */

.data
.balign 8
_str104:
	.ascii "check"
	.byte 0
/* end data */

.data
.balign 8
_str105:
	.ascii "Type check the project"
	.byte 0
/* end data */

.data
.balign 8
_str107:
	.ascii "clean"
	.byte 0
/* end data */

.data
.balign 8
_str108:
	.ascii "Delete build artifacts"
	.byte 0
/* end data */

.data
.balign 8
_str110:
	.ascii "docs"
	.byte 0
/* end data */

.data
.balign 8
_str111:
	.ascii "Generate HTML documentation"
	.byte 0
/* end data */

.data
.balign 8
_str113:
	.ascii "format"
	.byte 0
/* end data */

.data
.balign 8
_str114:
	.ascii "Format source files (alias: fmt)"
	.byte 0
/* end data */

.data
.balign 8
_str116:
	.ascii "new"
	.byte 0
/* end data */

.data
.balign 8
_str117:
	.ascii "Create a new project"
	.byte 0
/* end data */

.data
.balign 8
_str119:
	.ascii "run"
	.byte 0
/* end data */

.data
.balign 8
_str120:
	.ascii "Build and run the project"
	.byte 0
/* end data */

.data
.balign 8
_str122:
	.ascii "test"
	.byte 0
/* end data */

.data
.balign 8
_str123:
	.ascii "Run the project tests"
	.byte 0
/* end data */

.data
.balign 8
_str125:
	.ascii "version"
	.byte 0
/* end data */

.data
.balign 8
_str126:
	.ascii "Print the compiler version"
	.byte 0
/* end data */

.data
.balign 8
_str128:
	.ascii "help"
	.byte 0
/* end data */

.data
.balign 8
_str129:
	.ascii "Show help for Donna or a command"
	.byte 0
/* end data */

.data
.balign 8
_str161:
	.byte 10
	.byte 0
/* end data */

.data
.balign 8
_str163:
	.ascii "build"
	.byte 0
/* end data */

.data
.balign 8
_str164:
	.ascii "Build the project"
	.byte 0
/* end data */

.data
.balign 8
_str165:
	.ascii "donna build [DIR]"
	.byte 0
/* end data */

.data
.balign 8
_str166:
	.ascii "-h, --help"
	.byte 0
/* end data */

.data
.balign 8
_str167:
	.ascii "Show this help"
	.byte 0
/* end data */

.data
.balign 8
_str174:
	.ascii "DIR"
	.byte 0
/* end data */

.data
.balign 8
_str175:
	.ascii "Build another project directory"
	.byte 0
/* end data */

.data
.balign 8
_str183:
	.ascii "check"
	.byte 0
/* end data */

.data
.balign 8
_str184:
	.ascii "Type check the project entry module"
	.byte 0
/* end data */

.data
.balign 8
_str185:
	.ascii "donna check [DIR]"
	.byte 0
/* end data */

.data
.balign 8
_str186:
	.ascii "-h, --help"
	.byte 0
/* end data */

.data
.balign 8
_str187:
	.ascii "Show this help"
	.byte 0
/* end data */

.data
.balign 8
_str194:
	.ascii "DIR"
	.byte 0
/* end data */

.data
.balign 8
_str195:
	.ascii "Check another project directory"
	.byte 0
/* end data */

.data
.balign 8
_str203:
	.ascii "clean"
	.byte 0
/* end data */

.data
.balign 8
_str204:
	.ascii "Delete generated files"
	.byte 0
/* end data */

.data
.balign 8
_str205:
	.ascii "donna clean [DIR] [--docs] [--lock] [--all]"
	.byte 0
/* end data */

.data
.balign 8
_str206:
	.ascii "-h, --help"
	.byte 0
/* end data */

.data
.balign 8
_str207:
	.ascii "Show this help"
	.byte 0
/* end data */

.data
.balign 8
_str214:
	.ascii "--all"
	.byte 0
/* end data */

.data
.balign 8
_str215:
	.ascii "Delete build/, docs/, and donna.lock"
	.byte 0
/* end data */

.data
.balign 8
_str222:
	.ascii "--lock"
	.byte 0
/* end data */

.data
.balign 8
_str223:
	.ascii "Also delete donna.lock"
	.byte 0
/* end data */

.data
.balign 8
_str230:
	.ascii "--docs"
	.byte 0
/* end data */

.data
.balign 8
_str231:
	.ascii "Also delete generated docs/"
	.byte 0
/* end data */

.data
.balign 8
_str238:
	.ascii "DIR"
	.byte 0
/* end data */

.data
.balign 8
_str239:
	.ascii "Clean another project directory"
	.byte 0
/* end data */

.data
.balign 8
_str247:
	.ascii "docs"
	.byte 0
/* end data */

.data
.balign 8
_str248:
	.ascii "Generate HTML documentation"
	.byte 0
/* end data */

.data
.balign 8
_str249:
	.ascii "donna docs [DIR]"
	.byte 0
/* end data */

.data
.balign 8
_str250:
	.ascii "-h, --help"
	.byte 0
/* end data */

.data
.balign 8
_str251:
	.ascii "Show this help"
	.byte 0
/* end data */

.data
.balign 8
_str258:
	.ascii "DIR"
	.byte 0
/* end data */

.data
.balign 8
_str259:
	.ascii "Generate docs for another project directory"
	.byte 0
/* end data */

.data
.balign 8
_str267:
	.ascii "format"
	.byte 0
/* end data */

.data
.balign 8
_str268:
	.ascii "Format Donna source files"
	.byte 0
/* end data */

.data
.balign 8
_str269:
	.ascii "donna format [DIR|FILE]"
	.byte 0
/* end data */

.data
.balign 8
_str270:
	.ascii "-h, --help"
	.byte 0
/* end data */

.data
.balign 8
_str271:
	.ascii "Show this help"
	.byte 0
/* end data */

.data
.balign 8
_str278:
	.ascii "FILE"
	.byte 0
/* end data */

.data
.balign 8
_str279:
	.ascii "Format one .donna file"
	.byte 0
/* end data */

.data
.balign 8
_str286:
	.ascii "DIR"
	.byte 0
/* end data */

.data
.balign 8
_str287:
	.ascii "Format all .donna files in a project directory"
	.byte 0
/* end data */

.data
.balign 8
_str295:
	.ascii "new"
	.byte 0
/* end data */

.data
.balign 8
_str296:
	.ascii "Create a new Donna project"
	.byte 0
/* end data */

.data
.balign 8
_str297:
	.ascii "donna new <NAME>"
	.byte 0
/* end data */

.data
.balign 8
_str298:
	.ascii "-h, --help"
	.byte 0
/* end data */

.data
.balign 8
_str299:
	.ascii "Show this help"
	.byte 0
/* end data */

.data
.balign 8
_str306:
	.ascii "NAME"
	.byte 0
/* end data */

.data
.balign 8
_str307:
	.ascii "Directory and package name for the new project"
	.byte 0
/* end data */

.data
.balign 8
_str315:
	.ascii "run"
	.byte 0
/* end data */

.data
.balign 8
_str316:
	.ascii "Build and run an application project"
	.byte 0
/* end data */

.data
.balign 8
_str317:
	.ascii "donna run [DIR]"
	.byte 0
/* end data */

.data
.balign 8
_str318:
	.ascii "-h, --help"
	.byte 0
/* end data */

.data
.balign 8
_str319:
	.ascii "Show this help"
	.byte 0
/* end data */

.data
.balign 8
_str326:
	.ascii "DIR"
	.byte 0
/* end data */

.data
.balign 8
_str327:
	.ascii "Run another project directory"
	.byte 0
/* end data */

.data
.balign 8
_str335:
	.ascii "test"
	.byte 0
/* end data */

.data
.balign 8
_str336:
	.ascii "Run the project tests"
	.byte 0
/* end data */

.data
.balign 8
_str337:
	.ascii "donna test [DIR] [FILTER] | donna test --only FILTER"
	.byte 0
/* end data */

.data
.balign 8
_str338:
	.ascii "-h, --help"
	.byte 0
/* end data */

.data
.balign 8
_str339:
	.ascii "Show this help"
	.byte 0
/* end data */

.data
.balign 8
_str346:
	.ascii "DIR"
	.byte 0
/* end data */

.data
.balign 8
_str347:
	.ascii "Run tests in another project directory"
	.byte 0
/* end data */

.data
.balign 8
_str354:
	.ascii "FILTER"
	.byte 0
/* end data */

.data
.balign 8
_str355:
	.ascii "Run matching tests in the current project"
	.byte 0
/* end data */

.data
.balign 8
_str362:
	.ascii "--only FILTER"
	.byte 0
/* end data */

.data
.balign 8
_str363:
	.ascii "Run tests whose module or function name contains FILTER"
	.byte 0
/* end data */

.data
.balign 8
_str371:
	.ascii "version"
	.byte 0
/* end data */

.data
.balign 8
_str372:
	.ascii "Print the compiler version"
	.byte 0
/* end data */

.data
.balign 8
_str373:
	.ascii "donna version"
	.byte 0
/* end data */

.data
.balign 8
_str374:
	.ascii "-h, --help"
	.byte 0
/* end data */

.data
.balign 8
_str375:
	.ascii "Show this help"
	.byte 0
/* end data */

.data
.balign 8
_str382:
	.ascii "-V, --version"
	.byte 0
/* end data */

.data
.balign 8
_str383:
	.ascii "Print version from the top-level command"
	.byte 0
/* end data */

.data
.balign 8
_str391:
	.ascii "-h, --help"
	.byte 0
/* end data */

.data
.balign 8
_str392:
	.ascii "Print help"
	.byte 0
/* end data */

.data
.balign 8
_str397:
	.ascii "-V, --version"
	.byte 0
/* end data */

.data
.balign 8
_str398:
	.ascii "Print version"
	.byte 0
/* end data */

.data
.balign 8
_str409:
	.byte 10
	.byte 0
/* end data */

.data
.balign 8
_str411:
	.ascii "  "
	.byte 0
/* end data */

.data
.balign 8
_str422:
	.ascii "  "
	.byte 0
/* end data */

.data
.balign 8
_str438:
	.ascii " "
	.byte 0
/* end data */

.data
.balign 8
_str441:
	.ascii "  "
	.byte 0
/* end data */

.text
.balign 4
.globl _cli_help_main_help
_cli_help_main_help:
	hint	#34
	stp	x29, x30, [sp, -64]!
	mov	x29, sp
	str	x19, [x29, 56]
	str	x20, [x29, 48]
	str	x21, [x29, 40]
	str	x22, [x29, 32]
	str	x23, [x29, 24]
	str	x24, [x29, 16]
	mov	x19, x0
	adrp	x0, _str0@page
	add	x0, x0, _str0@pageoff
	bl	_utilities_colors_bold
	mov	x1, x19
	mov	x19, x1
	adrp	x1, _str2@page
	add	x1, x1, _str2@pageoff
	bl	___rt_str_concat
	mov	x1, x19
	bl	___rt_str_concat
	mov	x20, x0
	adrp	x0, _str5@page
	add	x0, x0, _str5@pageoff
	bl	_cli_help_header
	adrp	x1, _str7@page
	add	x1, x1, _str7@pageoff
	bl	___rt_str_concat
	mov	x19, x0
	adrp	x0, _str9@page
	add	x0, x0, _str9@pageoff
	bl	_utilities_colors_path
	mov	x1, x0
	mov	x0, x19
	bl	___rt_str_concat
	mov	x22, x0
	adrp	x0, _str12@page
	add	x0, x0, _str12@pageoff
	bl	_cli_help_header
	adrp	x1, _str14@page
	add	x1, x1, _str14@pageoff
	bl	___rt_str_concat
	mov	x19, x0
	bl	_cli_help_commands_block
	mov	x1, x0
	mov	x0, x19
	bl	___rt_str_concat
	mov	x23, x0
	adrp	x0, _str18@page
	add	x0, x0, _str18@pageoff
	bl	_cli_help_header
	adrp	x1, _str20@page
	add	x1, x1, _str20@pageoff
	bl	___rt_str_concat
	mov	x19, x0
	bl	_cli_help_options_block
	mov	x1, x0
	mov	x0, x19
	bl	___rt_str_concat
	mov	x24, x0
	mov	x0, #24
	bl	_malloc
	mov	x19, x0
	mov	x0, #1
	str	x0, [x19]
	mov	x0, #8
	add	x1, x19, x0
	adrp	x0, _str24@page
	add	x0, x0, _str24@pageoff
	str	x0, [x1]
	mov	x0, #16
	add	x1, x19, x0
	adrp	x0, _donna_nil@page
	add	x0, x0, _donna_nil@pageoff
	str	x0, [x1]
	mov	x0, #24
	bl	_malloc
	mov	x21, x0
	mov	x0, #1
	str	x0, [x21]
	mov	x0, #8
	add	x0, x21, x0
	str	x24, [x0]
	mov	x0, #16
	add	x0, x21, x0
	str	x19, [x0]
	mov	x0, #24
	bl	_malloc
	mov	x19, x0
	mov	x0, #1
	str	x0, [x19]
	mov	x0, #8
	add	x1, x19, x0
	adrp	x0, _str31@page
	add	x0, x0, _str31@pageoff
	str	x0, [x1]
	mov	x0, #16
	add	x0, x19, x0
	str	x21, [x0]
	mov	x0, #24
	bl	_malloc
	mov	x21, x0
	mov	x0, #1
	str	x0, [x21]
	mov	x0, #8
	add	x0, x21, x0
	str	x23, [x0]
	mov	x0, #16
	add	x0, x21, x0
	str	x19, [x0]
	mov	x0, #24
	bl	_malloc
	mov	x19, x0
	mov	x0, #1
	str	x0, [x19]
	mov	x0, #8
	add	x1, x19, x0
	adrp	x0, _str38@page
	add	x0, x0, _str38@pageoff
	str	x0, [x1]
	mov	x0, #16
	add	x0, x19, x0
	str	x21, [x0]
	mov	x0, #24
	bl	_malloc
	mov	x21, x0
	mov	x0, #1
	str	x0, [x21]
	mov	x0, #8
	add	x0, x21, x0
	str	x22, [x0]
	mov	x0, #16
	add	x0, x21, x0
	str	x19, [x0]
	mov	x0, #24
	bl	_malloc
	mov	x19, x0
	mov	x0, #1
	str	x0, [x19]
	mov	x0, #8
	add	x1, x19, x0
	adrp	x0, _str45@page
	add	x0, x0, _str45@pageoff
	str	x0, [x1]
	mov	x0, #16
	add	x0, x19, x0
	str	x21, [x0]
	mov	x0, #24
	bl	_malloc
	mov	x1, #1
	str	x1, [x0]
	mov	x1, #8
	add	x1, x0, x1
	str	x20, [x1]
	mov	x1, #16
	add	x1, x0, x1
	str	x19, [x1]
	adrp	x1, _str52@page
	add	x1, x1, _str52@pageoff
	bl	_donna_string_join
	ldr	x19, [x29, 56]
	ldr	x20, [x29, 48]
	ldr	x21, [x29, 40]
	ldr	x22, [x29, 32]
	ldr	x23, [x29, 24]
	ldr	x24, [x29, 16]
	ldp	x29, x30, [sp], 64
	ret
/* end function cli_help_main_help */

.text
.balign 4
.globl _cli_help_command_help
_cli_help_command_help:
	hint	#34
	stp	x29, x30, [sp, -64]!
	mov	x29, sp
	str	x19, [x29, 56]
	str	x20, [x29, 48]
	str	x21, [x29, 40]
	str	x22, [x29, 32]
	str	x23, [x29, 24]
	mov	x21, x2
	mov	x20, x1
	mov	x1, x0
	mov	x0, x3
	mov	x19, x0
	adrp	x0, _str54@page
	add	x0, x0, _str54@pageoff
	bl	___rt_str_concat
	mov	x1, x20
	mov	x20, x1
	adrp	x1, _str56@page
	add	x1, x1, _str56@pageoff
	bl	___rt_str_concat
	mov	x1, x20
	bl	___rt_str_concat
	mov	x20, x0
	mov	x0, x19
	mov	x19, x0
	adrp	x0, _str59@page
	add	x0, x0, _str59@pageoff
	bl	_cli_help_header
	mov	x1, x21
	mov	x21, x1
	adrp	x1, _str61@page
	add	x1, x1, _str61@pageoff
	bl	___rt_str_concat
	mov	x1, x21
	bl	___rt_str_concat
	mov	x22, x0
	mov	x0, x19
	mov	x19, x0
	bl	_donna_list_is_empty
	mov	x1, x0
	mov	x0, x19
	cmp	x1, #1
	beq	L4
	mov	x19, x0
	adrp	x0, _str72@page
	add	x0, x0, _str72@pageoff
	bl	_cli_help_header
	mov	x1, x0
	mov	x0, x19
	mov	x19, x0
	adrp	x0, _str71@page
	add	x0, x0, _str71@pageoff
	bl	___rt_str_concat
	adrp	x1, _str75@page
	add	x1, x1, _str75@pageoff
	bl	___rt_str_concat
	mov	x17, x0
	mov	x0, x19
	mov	x19, x17
	adrp	x1, _cli_help_option_row@page
	add	x1, x1, _cli_help_option_row@pageoff
	bl	_donna_list_map
	adrp	x1, _str78@page
	add	x1, x1, _str78@pageoff
	bl	_donna_string_join
	mov	x1, x0
	mov	x0, x19
	bl	___rt_str_concat
	mov	x23, x0
	b	L5
L4:
	adrp	x23, _str70@page
	add	x23, x23, _str70@pageoff
L5:
	mov	x0, #24
	bl	_malloc
	mov	x21, x0
	mov	x0, #1
	str	x0, [x21]
	mov	x0, #8
	add	x1, x21, x0
	adrp	x0, _str82@page
	add	x0, x0, _str82@pageoff
	str	x0, [x1]
	mov	x0, #16
	add	x1, x21, x0
	adrp	x0, _donna_nil@page
	add	x0, x0, _donna_nil@pageoff
	str	x0, [x1]
	mov	x0, #24
	bl	_malloc
	mov	x19, x0
	mov	x0, #1
	str	x0, [x19]
	mov	x0, #8
	add	x0, x19, x0
	str	x23, [x0]
	mov	x0, #16
	add	x0, x19, x0
	str	x21, [x0]
	mov	x0, #24
	bl	_malloc
	mov	x21, x0
	mov	x0, #1
	str	x0, [x21]
	mov	x0, #8
	add	x0, x21, x0
	str	x22, [x0]
	mov	x0, #16
	add	x0, x21, x0
	str	x19, [x0]
	mov	x0, #24
	bl	_malloc
	mov	x19, x0
	mov	x0, #1
	str	x0, [x19]
	mov	x0, #8
	add	x1, x19, x0
	adrp	x0, _str92@page
	add	x0, x0, _str92@pageoff
	str	x0, [x1]
	mov	x0, #16
	add	x0, x19, x0
	str	x21, [x0]
	mov	x0, #24
	bl	_malloc
	mov	x1, #1
	str	x1, [x0]
	mov	x1, #8
	add	x1, x0, x1
	str	x20, [x1]
	mov	x1, #16
	add	x1, x0, x1
	str	x19, [x1]
	adrp	x1, _str99@page
	add	x1, x1, _str99@pageoff
	bl	_donna_string_join
	ldr	x19, [x29, 56]
	ldr	x20, [x29, 48]
	ldr	x21, [x29, 40]
	ldr	x22, [x29, 32]
	ldr	x23, [x29, 24]
	ldp	x29, x30, [sp], 64
	ret
/* end function cli_help_command_help */

.text
.balign 4
_cli_help_commands_block:
	hint	#34
	stp	x29, x30, [sp, -112]!
	mov	x29, sp
	str	x19, [x29, 104]
	str	x20, [x29, 96]
	str	x21, [x29, 88]
	str	x22, [x29, 80]
	str	x23, [x29, 72]
	str	x24, [x29, 64]
	str	x25, [x29, 56]
	str	x26, [x29, 48]
	str	x27, [x29, 40]
	adrp	x1, _str102@page
	add	x1, x1, _str102@pageoff
	adrp	x0, _str101@page
	add	x0, x0, _str101@pageoff
	bl	_cli_help_command_row
	mov	x20, x0
	adrp	x1, _str105@page
	add	x1, x1, _str105@pageoff
	adrp	x0, _str104@page
	add	x0, x0, _str104@pageoff
	bl	_cli_help_command_row
	mov	x22, x0
	adrp	x1, _str108@page
	add	x1, x1, _str108@pageoff
	adrp	x0, _str107@page
	add	x0, x0, _str107@pageoff
	bl	_cli_help_command_row
	mov	x23, x0
	adrp	x1, _str111@page
	add	x1, x1, _str111@pageoff
	adrp	x0, _str110@page
	add	x0, x0, _str110@pageoff
	bl	_cli_help_command_row
	mov	x24, x0
	adrp	x1, _str114@page
	add	x1, x1, _str114@pageoff
	adrp	x0, _str113@page
	add	x0, x0, _str113@pageoff
	bl	_cli_help_command_row
	mov	x25, x0
	adrp	x1, _str117@page
	add	x1, x1, _str117@pageoff
	adrp	x0, _str116@page
	add	x0, x0, _str116@pageoff
	bl	_cli_help_command_row
	mov	x26, x0
	adrp	x1, _str120@page
	add	x1, x1, _str120@pageoff
	adrp	x0, _str119@page
	add	x0, x0, _str119@pageoff
	bl	_cli_help_command_row
	mov	x27, x0
	adrp	x1, _str123@page
	add	x1, x1, _str123@pageoff
	adrp	x0, _str122@page
	add	x0, x0, _str122@pageoff
	bl	_cli_help_command_row
	str	x0, [x29, 16]
	adrp	x1, _str126@page
	add	x1, x1, _str126@pageoff
	adrp	x0, _str125@page
	add	x0, x0, _str125@pageoff
	bl	_cli_help_command_row
	str	x0, [x29, 24]
	adrp	x1, _str129@page
	add	x1, x1, _str129@pageoff
	adrp	x0, _str128@page
	add	x0, x0, _str128@pageoff
	bl	_cli_help_command_row
	mov	x21, x0
	mov	x0, #24
	bl	_malloc
	mov	x19, x0
	mov	x0, #1
	str	x0, [x19]
	mov	x0, #8
	add	x0, x19, x0
	str	x21, [x0]
	mov	x0, #16
	add	x1, x19, x0
	adrp	x0, _donna_nil@page
	add	x0, x0, _donna_nil@pageoff
	str	x0, [x1]
	mov	x0, #24
	bl	_malloc
	mov	x21, x0
	ldr	x0, [x29, 24]
	mov	x1, #1
	str	x1, [x21]
	mov	x1, #8
	add	x1, x21, x1
	str	x0, [x1]
	mov	x0, #16
	add	x0, x21, x0
	str	x19, [x0]
	mov	x0, #24
	bl	_malloc
	mov	x19, x0
	ldr	x0, [x29, 16]
	mov	x1, #1
	str	x1, [x19]
	mov	x1, #8
	add	x1, x19, x1
	str	x0, [x1]
	mov	x0, #16
	add	x0, x19, x0
	str	x21, [x0]
	mov	x0, #24
	bl	_malloc
	mov	x21, x0
	mov	x0, #1
	str	x0, [x21]
	mov	x0, #8
	add	x0, x21, x0
	str	x27, [x0]
	mov	x0, #16
	add	x0, x21, x0
	str	x19, [x0]
	mov	x0, #24
	bl	_malloc
	mov	x19, x0
	mov	x0, #1
	str	x0, [x19]
	mov	x0, #8
	add	x0, x19, x0
	str	x26, [x0]
	mov	x0, #16
	add	x0, x19, x0
	str	x21, [x0]
	mov	x0, #24
	bl	_malloc
	mov	x21, x0
	mov	x0, #1
	str	x0, [x21]
	mov	x0, #8
	add	x0, x21, x0
	str	x25, [x0]
	mov	x0, #16
	add	x0, x21, x0
	str	x19, [x0]
	mov	x0, #24
	bl	_malloc
	mov	x19, x0
	mov	x0, #1
	str	x0, [x19]
	mov	x0, #8
	add	x0, x19, x0
	str	x24, [x0]
	mov	x0, #16
	add	x0, x19, x0
	str	x21, [x0]
	mov	x0, #24
	bl	_malloc
	mov	x21, x0
	mov	x0, #1
	str	x0, [x21]
	mov	x0, #8
	add	x0, x21, x0
	str	x23, [x0]
	mov	x0, #16
	add	x0, x21, x0
	str	x19, [x0]
	mov	x0, #24
	bl	_malloc
	mov	x19, x0
	mov	x0, #1
	str	x0, [x19]
	mov	x0, #8
	add	x0, x19, x0
	str	x22, [x0]
	mov	x0, #16
	add	x0, x19, x0
	str	x21, [x0]
	mov	x0, #24
	bl	_malloc
	mov	x1, #1
	str	x1, [x0]
	mov	x1, #8
	add	x1, x0, x1
	str	x20, [x1]
	mov	x1, #16
	add	x1, x0, x1
	str	x19, [x1]
	adrp	x1, _str161@page
	add	x1, x1, _str161@pageoff
	bl	_donna_string_join
	ldr	x19, [x29, 104]
	ldr	x20, [x29, 96]
	ldr	x21, [x29, 88]
	ldr	x22, [x29, 80]
	ldr	x23, [x29, 72]
	ldr	x24, [x29, 64]
	ldr	x25, [x29, 56]
	ldr	x26, [x29, 48]
	ldr	x27, [x29, 40]
	ldp	x29, x30, [sp], 112
	ret
/* end function cli_help_commands_block */

.text
.balign 4
.globl _cli_help_build_help
_cli_help_build_help:
	hint	#34
	stp	x29, x30, [sp, -32]!
	mov	x29, sp
	str	x19, [x29, 24]
	str	x20, [x29, 16]
	mov	x0, #16
	bl	_malloc
	mov	x20, x0
	adrp	x0, _str166@page
	add	x0, x0, _str166@pageoff
	str	x0, [x20]
	mov	x0, #8
	add	x1, x20, x0
	adrp	x0, _str167@page
	add	x0, x0, _str167@pageoff
	str	x0, [x1]
	mov	x0, #24
	bl	_malloc
	mov	x19, x0
	mov	x0, #1
	str	x0, [x19]
	mov	x0, #8
	add	x0, x19, x0
	str	x20, [x0]
	mov	x0, #16
	add	x1, x19, x0
	adrp	x0, _donna_nil@page
	add	x0, x0, _donna_nil@pageoff
	str	x0, [x1]
	mov	x0, #16
	bl	_malloc
	mov	x20, x0
	adrp	x0, _str174@page
	add	x0, x0, _str174@pageoff
	str	x0, [x20]
	mov	x0, #8
	add	x1, x20, x0
	adrp	x0, _str175@page
	add	x0, x0, _str175@pageoff
	str	x0, [x1]
	mov	x0, #24
	bl	_malloc
	mov	x3, x0
	mov	x0, #1
	str	x0, [x3]
	mov	x0, #8
	add	x0, x3, x0
	str	x20, [x0]
	mov	x0, #16
	add	x0, x3, x0
	str	x19, [x0]
	adrp	x2, _str165@page
	add	x2, x2, _str165@pageoff
	adrp	x1, _str164@page
	add	x1, x1, _str164@pageoff
	adrp	x0, _str163@page
	add	x0, x0, _str163@pageoff
	bl	_cli_help_command_help
	ldr	x19, [x29, 24]
	ldr	x20, [x29, 16]
	ldp	x29, x30, [sp], 32
	ret
/* end function cli_help_build_help */

.text
.balign 4
.globl _cli_help_check_help
_cli_help_check_help:
	hint	#34
	stp	x29, x30, [sp, -32]!
	mov	x29, sp
	str	x19, [x29, 24]
	str	x20, [x29, 16]
	mov	x0, #16
	bl	_malloc
	mov	x20, x0
	adrp	x0, _str186@page
	add	x0, x0, _str186@pageoff
	str	x0, [x20]
	mov	x0, #8
	add	x1, x20, x0
	adrp	x0, _str187@page
	add	x0, x0, _str187@pageoff
	str	x0, [x1]
	mov	x0, #24
	bl	_malloc
	mov	x19, x0
	mov	x0, #1
	str	x0, [x19]
	mov	x0, #8
	add	x0, x19, x0
	str	x20, [x0]
	mov	x0, #16
	add	x1, x19, x0
	adrp	x0, _donna_nil@page
	add	x0, x0, _donna_nil@pageoff
	str	x0, [x1]
	mov	x0, #16
	bl	_malloc
	mov	x20, x0
	adrp	x0, _str194@page
	add	x0, x0, _str194@pageoff
	str	x0, [x20]
	mov	x0, #8
	add	x1, x20, x0
	adrp	x0, _str195@page
	add	x0, x0, _str195@pageoff
	str	x0, [x1]
	mov	x0, #24
	bl	_malloc
	mov	x3, x0
	mov	x0, #1
	str	x0, [x3]
	mov	x0, #8
	add	x0, x3, x0
	str	x20, [x0]
	mov	x0, #16
	add	x0, x3, x0
	str	x19, [x0]
	adrp	x2, _str185@page
	add	x2, x2, _str185@pageoff
	adrp	x1, _str184@page
	add	x1, x1, _str184@pageoff
	adrp	x0, _str183@page
	add	x0, x0, _str183@pageoff
	bl	_cli_help_command_help
	ldr	x19, [x29, 24]
	ldr	x20, [x29, 16]
	ldp	x29, x30, [sp], 32
	ret
/* end function cli_help_check_help */

.text
.balign 4
.globl _cli_help_clean_help
_cli_help_clean_help:
	hint	#34
	stp	x29, x30, [sp, -48]!
	mov	x29, sp
	str	x19, [x29, 40]
	str	x20, [x29, 32]
	str	x21, [x29, 24]
	mov	x0, #16
	bl	_malloc
	mov	x19, x0
	adrp	x0, _str206@page
	add	x0, x0, _str206@pageoff
	str	x0, [x19]
	mov	x0, #8
	add	x1, x19, x0
	adrp	x0, _str207@page
	add	x0, x0, _str207@pageoff
	str	x0, [x1]
	mov	x0, #24
	bl	_malloc
	mov	x20, x0
	mov	x0, #1
	str	x0, [x20]
	mov	x0, #8
	add	x0, x20, x0
	str	x19, [x0]
	mov	x0, #16
	add	x1, x20, x0
	adrp	x0, _donna_nil@page
	add	x0, x0, _donna_nil@pageoff
	str	x0, [x1]
	mov	x0, #16
	bl	_malloc
	mov	x21, x0
	adrp	x0, _str214@page
	add	x0, x0, _str214@pageoff
	str	x0, [x21]
	mov	x0, #8
	add	x1, x21, x0
	adrp	x0, _str215@page
	add	x0, x0, _str215@pageoff
	str	x0, [x1]
	mov	x0, #24
	bl	_malloc
	mov	x19, x0
	mov	x0, #1
	str	x0, [x19]
	mov	x0, #8
	add	x0, x19, x0
	str	x21, [x0]
	mov	x0, #16
	add	x0, x19, x0
	str	x20, [x0]
	mov	x0, #16
	bl	_malloc
	mov	x21, x0
	adrp	x0, _str222@page
	add	x0, x0, _str222@pageoff
	str	x0, [x21]
	mov	x0, #8
	add	x1, x21, x0
	adrp	x0, _str223@page
	add	x0, x0, _str223@pageoff
	str	x0, [x1]
	mov	x0, #24
	bl	_malloc
	mov	x20, x0
	mov	x0, #1
	str	x0, [x20]
	mov	x0, #8
	add	x0, x20, x0
	str	x21, [x0]
	mov	x0, #16
	add	x0, x20, x0
	str	x19, [x0]
	mov	x0, #16
	bl	_malloc
	mov	x21, x0
	adrp	x0, _str230@page
	add	x0, x0, _str230@pageoff
	str	x0, [x21]
	mov	x0, #8
	add	x1, x21, x0
	adrp	x0, _str231@page
	add	x0, x0, _str231@pageoff
	str	x0, [x1]
	mov	x0, #24
	bl	_malloc
	mov	x19, x0
	mov	x0, #1
	str	x0, [x19]
	mov	x0, #8
	add	x0, x19, x0
	str	x21, [x0]
	mov	x0, #16
	add	x0, x19, x0
	str	x20, [x0]
	mov	x0, #16
	bl	_malloc
	mov	x20, x0
	adrp	x0, _str238@page
	add	x0, x0, _str238@pageoff
	str	x0, [x20]
	mov	x0, #8
	add	x1, x20, x0
	adrp	x0, _str239@page
	add	x0, x0, _str239@pageoff
	str	x0, [x1]
	mov	x0, #24
	bl	_malloc
	mov	x3, x0
	mov	x0, #1
	str	x0, [x3]
	mov	x0, #8
	add	x0, x3, x0
	str	x20, [x0]
	mov	x0, #16
	add	x0, x3, x0
	str	x19, [x0]
	adrp	x2, _str205@page
	add	x2, x2, _str205@pageoff
	adrp	x1, _str204@page
	add	x1, x1, _str204@pageoff
	adrp	x0, _str203@page
	add	x0, x0, _str203@pageoff
	bl	_cli_help_command_help
	ldr	x19, [x29, 40]
	ldr	x20, [x29, 32]
	ldr	x21, [x29, 24]
	ldp	x29, x30, [sp], 48
	ret
/* end function cli_help_clean_help */

.text
.balign 4
.globl _cli_help_docs_help
_cli_help_docs_help:
	hint	#34
	stp	x29, x30, [sp, -32]!
	mov	x29, sp
	str	x19, [x29, 24]
	str	x20, [x29, 16]
	mov	x0, #16
	bl	_malloc
	mov	x20, x0
	adrp	x0, _str250@page
	add	x0, x0, _str250@pageoff
	str	x0, [x20]
	mov	x0, #8
	add	x1, x20, x0
	adrp	x0, _str251@page
	add	x0, x0, _str251@pageoff
	str	x0, [x1]
	mov	x0, #24
	bl	_malloc
	mov	x19, x0
	mov	x0, #1
	str	x0, [x19]
	mov	x0, #8
	add	x0, x19, x0
	str	x20, [x0]
	mov	x0, #16
	add	x1, x19, x0
	adrp	x0, _donna_nil@page
	add	x0, x0, _donna_nil@pageoff
	str	x0, [x1]
	mov	x0, #16
	bl	_malloc
	mov	x20, x0
	adrp	x0, _str258@page
	add	x0, x0, _str258@pageoff
	str	x0, [x20]
	mov	x0, #8
	add	x1, x20, x0
	adrp	x0, _str259@page
	add	x0, x0, _str259@pageoff
	str	x0, [x1]
	mov	x0, #24
	bl	_malloc
	mov	x3, x0
	mov	x0, #1
	str	x0, [x3]
	mov	x0, #8
	add	x0, x3, x0
	str	x20, [x0]
	mov	x0, #16
	add	x0, x3, x0
	str	x19, [x0]
	adrp	x2, _str249@page
	add	x2, x2, _str249@pageoff
	adrp	x1, _str248@page
	add	x1, x1, _str248@pageoff
	adrp	x0, _str247@page
	add	x0, x0, _str247@pageoff
	bl	_cli_help_command_help
	ldr	x19, [x29, 24]
	ldr	x20, [x29, 16]
	ldp	x29, x30, [sp], 32
	ret
/* end function cli_help_docs_help */

.text
.balign 4
.globl _cli_help_format_help
_cli_help_format_help:
	hint	#34
	stp	x29, x30, [sp, -48]!
	mov	x29, sp
	str	x19, [x29, 40]
	str	x20, [x29, 32]
	str	x21, [x29, 24]
	mov	x0, #16
	bl	_malloc
	mov	x19, x0
	adrp	x0, _str270@page
	add	x0, x0, _str270@pageoff
	str	x0, [x19]
	mov	x0, #8
	add	x1, x19, x0
	adrp	x0, _str271@page
	add	x0, x0, _str271@pageoff
	str	x0, [x1]
	mov	x0, #24
	bl	_malloc
	mov	x20, x0
	mov	x0, #1
	str	x0, [x20]
	mov	x0, #8
	add	x0, x20, x0
	str	x19, [x0]
	mov	x0, #16
	add	x1, x20, x0
	adrp	x0, _donna_nil@page
	add	x0, x0, _donna_nil@pageoff
	str	x0, [x1]
	mov	x0, #16
	bl	_malloc
	mov	x21, x0
	adrp	x0, _str278@page
	add	x0, x0, _str278@pageoff
	str	x0, [x21]
	mov	x0, #8
	add	x1, x21, x0
	adrp	x0, _str279@page
	add	x0, x0, _str279@pageoff
	str	x0, [x1]
	mov	x0, #24
	bl	_malloc
	mov	x19, x0
	mov	x0, #1
	str	x0, [x19]
	mov	x0, #8
	add	x0, x19, x0
	str	x21, [x0]
	mov	x0, #16
	add	x0, x19, x0
	str	x20, [x0]
	mov	x0, #16
	bl	_malloc
	mov	x20, x0
	adrp	x0, _str286@page
	add	x0, x0, _str286@pageoff
	str	x0, [x20]
	mov	x0, #8
	add	x1, x20, x0
	adrp	x0, _str287@page
	add	x0, x0, _str287@pageoff
	str	x0, [x1]
	mov	x0, #24
	bl	_malloc
	mov	x3, x0
	mov	x0, #1
	str	x0, [x3]
	mov	x0, #8
	add	x0, x3, x0
	str	x20, [x0]
	mov	x0, #16
	add	x0, x3, x0
	str	x19, [x0]
	adrp	x2, _str269@page
	add	x2, x2, _str269@pageoff
	adrp	x1, _str268@page
	add	x1, x1, _str268@pageoff
	adrp	x0, _str267@page
	add	x0, x0, _str267@pageoff
	bl	_cli_help_command_help
	ldr	x19, [x29, 40]
	ldr	x20, [x29, 32]
	ldr	x21, [x29, 24]
	ldp	x29, x30, [sp], 48
	ret
/* end function cli_help_format_help */

.text
.balign 4
.globl _cli_help_new_help
_cli_help_new_help:
	hint	#34
	stp	x29, x30, [sp, -32]!
	mov	x29, sp
	str	x19, [x29, 24]
	str	x20, [x29, 16]
	mov	x0, #16
	bl	_malloc
	mov	x20, x0
	adrp	x0, _str298@page
	add	x0, x0, _str298@pageoff
	str	x0, [x20]
	mov	x0, #8
	add	x1, x20, x0
	adrp	x0, _str299@page
	add	x0, x0, _str299@pageoff
	str	x0, [x1]
	mov	x0, #24
	bl	_malloc
	mov	x19, x0
	mov	x0, #1
	str	x0, [x19]
	mov	x0, #8
	add	x0, x19, x0
	str	x20, [x0]
	mov	x0, #16
	add	x1, x19, x0
	adrp	x0, _donna_nil@page
	add	x0, x0, _donna_nil@pageoff
	str	x0, [x1]
	mov	x0, #16
	bl	_malloc
	mov	x20, x0
	adrp	x0, _str306@page
	add	x0, x0, _str306@pageoff
	str	x0, [x20]
	mov	x0, #8
	add	x1, x20, x0
	adrp	x0, _str307@page
	add	x0, x0, _str307@pageoff
	str	x0, [x1]
	mov	x0, #24
	bl	_malloc
	mov	x3, x0
	mov	x0, #1
	str	x0, [x3]
	mov	x0, #8
	add	x0, x3, x0
	str	x20, [x0]
	mov	x0, #16
	add	x0, x3, x0
	str	x19, [x0]
	adrp	x2, _str297@page
	add	x2, x2, _str297@pageoff
	adrp	x1, _str296@page
	add	x1, x1, _str296@pageoff
	adrp	x0, _str295@page
	add	x0, x0, _str295@pageoff
	bl	_cli_help_command_help
	ldr	x19, [x29, 24]
	ldr	x20, [x29, 16]
	ldp	x29, x30, [sp], 32
	ret
/* end function cli_help_new_help */

.text
.balign 4
.globl _cli_help_run_help
_cli_help_run_help:
	hint	#34
	stp	x29, x30, [sp, -32]!
	mov	x29, sp
	str	x19, [x29, 24]
	str	x20, [x29, 16]
	mov	x0, #16
	bl	_malloc
	mov	x20, x0
	adrp	x0, _str318@page
	add	x0, x0, _str318@pageoff
	str	x0, [x20]
	mov	x0, #8
	add	x1, x20, x0
	adrp	x0, _str319@page
	add	x0, x0, _str319@pageoff
	str	x0, [x1]
	mov	x0, #24
	bl	_malloc
	mov	x19, x0
	mov	x0, #1
	str	x0, [x19]
	mov	x0, #8
	add	x0, x19, x0
	str	x20, [x0]
	mov	x0, #16
	add	x1, x19, x0
	adrp	x0, _donna_nil@page
	add	x0, x0, _donna_nil@pageoff
	str	x0, [x1]
	mov	x0, #16
	bl	_malloc
	mov	x20, x0
	adrp	x0, _str326@page
	add	x0, x0, _str326@pageoff
	str	x0, [x20]
	mov	x0, #8
	add	x1, x20, x0
	adrp	x0, _str327@page
	add	x0, x0, _str327@pageoff
	str	x0, [x1]
	mov	x0, #24
	bl	_malloc
	mov	x3, x0
	mov	x0, #1
	str	x0, [x3]
	mov	x0, #8
	add	x0, x3, x0
	str	x20, [x0]
	mov	x0, #16
	add	x0, x3, x0
	str	x19, [x0]
	adrp	x2, _str317@page
	add	x2, x2, _str317@pageoff
	adrp	x1, _str316@page
	add	x1, x1, _str316@pageoff
	adrp	x0, _str315@page
	add	x0, x0, _str315@pageoff
	bl	_cli_help_command_help
	ldr	x19, [x29, 24]
	ldr	x20, [x29, 16]
	ldp	x29, x30, [sp], 32
	ret
/* end function cli_help_run_help */

.text
.balign 4
.globl _cli_help_test_help
_cli_help_test_help:
	hint	#34
	stp	x29, x30, [sp, -48]!
	mov	x29, sp
	str	x19, [x29, 40]
	str	x20, [x29, 32]
	str	x21, [x29, 24]
	mov	x0, #16
	bl	_malloc
	mov	x20, x0
	adrp	x0, _str338@page
	add	x0, x0, _str338@pageoff
	str	x0, [x20]
	mov	x0, #8
	add	x1, x20, x0
	adrp	x0, _str339@page
	add	x0, x0, _str339@pageoff
	str	x0, [x1]
	mov	x0, #24
	bl	_malloc
	mov	x19, x0
	mov	x0, #1
	str	x0, [x19]
	mov	x0, #8
	add	x0, x19, x0
	str	x20, [x0]
	mov	x0, #16
	add	x1, x19, x0
	adrp	x0, _donna_nil@page
	add	x0, x0, _donna_nil@pageoff
	str	x0, [x1]
	mov	x0, #16
	bl	_malloc
	mov	x21, x0
	adrp	x0, _str346@page
	add	x0, x0, _str346@pageoff
	str	x0, [x21]
	mov	x0, #8
	add	x1, x21, x0
	adrp	x0, _str347@page
	add	x0, x0, _str347@pageoff
	str	x0, [x1]
	mov	x0, #24
	bl	_malloc
	mov	x20, x0
	mov	x0, #1
	str	x0, [x20]
	mov	x0, #8
	add	x0, x20, x0
	str	x21, [x0]
	mov	x0, #16
	add	x0, x20, x0
	str	x19, [x0]
	mov	x0, #16
	bl	_malloc
	mov	x21, x0
	adrp	x0, _str354@page
	add	x0, x0, _str354@pageoff
	str	x0, [x21]
	mov	x0, #8
	add	x1, x21, x0
	adrp	x0, _str355@page
	add	x0, x0, _str355@pageoff
	str	x0, [x1]
	mov	x0, #24
	bl	_malloc
	mov	x19, x0
	mov	x0, #1
	str	x0, [x19]
	mov	x0, #8
	add	x0, x19, x0
	str	x21, [x0]
	mov	x0, #16
	add	x0, x19, x0
	str	x20, [x0]
	mov	x0, #16
	bl	_malloc
	mov	x20, x0
	adrp	x0, _str362@page
	add	x0, x0, _str362@pageoff
	str	x0, [x20]
	mov	x0, #8
	add	x1, x20, x0
	adrp	x0, _str363@page
	add	x0, x0, _str363@pageoff
	str	x0, [x1]
	mov	x0, #24
	bl	_malloc
	mov	x3, x0
	mov	x0, #1
	str	x0, [x3]
	mov	x0, #8
	add	x0, x3, x0
	str	x20, [x0]
	mov	x0, #16
	add	x0, x3, x0
	str	x19, [x0]
	adrp	x2, _str337@page
	add	x2, x2, _str337@pageoff
	adrp	x1, _str336@page
	add	x1, x1, _str336@pageoff
	adrp	x0, _str335@page
	add	x0, x0, _str335@pageoff
	bl	_cli_help_command_help
	ldr	x19, [x29, 40]
	ldr	x20, [x29, 32]
	ldr	x21, [x29, 24]
	ldp	x29, x30, [sp], 48
	ret
/* end function cli_help_test_help */

.text
.balign 4
.globl _cli_help_version_help
_cli_help_version_help:
	hint	#34
	stp	x29, x30, [sp, -32]!
	mov	x29, sp
	str	x19, [x29, 24]
	str	x20, [x29, 16]
	mov	x0, #16
	bl	_malloc
	mov	x20, x0
	adrp	x0, _str374@page
	add	x0, x0, _str374@pageoff
	str	x0, [x20]
	mov	x0, #8
	add	x1, x20, x0
	adrp	x0, _str375@page
	add	x0, x0, _str375@pageoff
	str	x0, [x1]
	mov	x0, #24
	bl	_malloc
	mov	x19, x0
	mov	x0, #1
	str	x0, [x19]
	mov	x0, #8
	add	x0, x19, x0
	str	x20, [x0]
	mov	x0, #16
	add	x1, x19, x0
	adrp	x0, _donna_nil@page
	add	x0, x0, _donna_nil@pageoff
	str	x0, [x1]
	mov	x0, #16
	bl	_malloc
	mov	x20, x0
	adrp	x0, _str382@page
	add	x0, x0, _str382@pageoff
	str	x0, [x20]
	mov	x0, #8
	add	x1, x20, x0
	adrp	x0, _str383@page
	add	x0, x0, _str383@pageoff
	str	x0, [x1]
	mov	x0, #24
	bl	_malloc
	mov	x3, x0
	mov	x0, #1
	str	x0, [x3]
	mov	x0, #8
	add	x0, x3, x0
	str	x20, [x0]
	mov	x0, #16
	add	x0, x3, x0
	str	x19, [x0]
	adrp	x2, _str373@page
	add	x2, x2, _str373@pageoff
	adrp	x1, _str372@page
	add	x1, x1, _str372@pageoff
	adrp	x0, _str371@page
	add	x0, x0, _str371@pageoff
	bl	_cli_help_command_help
	ldr	x19, [x29, 24]
	ldr	x20, [x29, 16]
	ldp	x29, x30, [sp], 32
	ret
/* end function cli_help_version_help */

.text
.balign 4
_cli_help_options_block:
	hint	#34
	stp	x29, x30, [sp, -48]!
	mov	x29, sp
	str	x19, [x29, 40]
	str	x20, [x29, 32]
	str	x21, [x29, 24]
	mov	x0, #16
	bl	_malloc
	adrp	x1, _str391@page
	add	x1, x1, _str391@pageoff
	str	x1, [x0]
	mov	x1, #8
	add	x2, x0, x1
	adrp	x1, _str392@page
	add	x1, x1, _str392@pageoff
	str	x1, [x2]
	bl	_cli_help_option_row
	mov	x20, x0
	mov	x0, #16
	bl	_malloc
	adrp	x1, _str397@page
	add	x1, x1, _str397@pageoff
	str	x1, [x0]
	mov	x1, #8
	add	x2, x0, x1
	adrp	x1, _str398@page
	add	x1, x1, _str398@pageoff
	str	x1, [x2]
	bl	_cli_help_option_row
	mov	x21, x0
	mov	x0, #24
	bl	_malloc
	mov	x19, x0
	mov	x0, #1
	str	x0, [x19]
	mov	x0, #8
	add	x0, x19, x0
	str	x21, [x0]
	mov	x0, #16
	add	x1, x19, x0
	adrp	x0, _donna_nil@page
	add	x0, x0, _donna_nil@pageoff
	str	x0, [x1]
	mov	x0, #24
	bl	_malloc
	mov	x1, #1
	str	x1, [x0]
	mov	x1, #8
	add	x1, x0, x1
	str	x20, [x1]
	mov	x1, #16
	add	x1, x0, x1
	str	x19, [x1]
	adrp	x1, _str409@page
	add	x1, x1, _str409@pageoff
	bl	_donna_string_join
	ldr	x19, [x29, 40]
	ldr	x20, [x29, 32]
	ldr	x21, [x29, 24]
	ldp	x29, x30, [sp], 48
	ret
/* end function cli_help_options_block */

.text
.balign 4
_cli_help_command_row:
	hint	#34
	stp	x29, x30, [sp, -32]!
	mov	x29, sp
	str	x19, [x29, 24]
	mov	x19, x1
	bl	_utilities_colors_path
	bl	_cli_help_pad_right
	mov	x1, x0
	adrp	x0, _str411@page
	add	x0, x0, _str411@pageoff
	bl	___rt_str_concat
	mov	x1, x19
	bl	___rt_str_concat
	ldr	x19, [x29, 24]
	ldp	x29, x30, [sp], 32
	ret
/* end function cli_help_command_row */

.text
.balign 4
_cli_help_option_row:
	hint	#34
	stp	x29, x30, [sp, -32]!
	mov	x29, sp
	str	x19, [x29, 24]
	mov	x1, x0
	ldr	x0, [x1]
	mov	x2, #8
	add	x1, x1, x2
	ldr	x19, [x1]
	bl	_utilities_colors_path
	bl	_cli_help_pad_right
	mov	x1, x0
	adrp	x0, _str422@page
	add	x0, x0, _str422@pageoff
	bl	___rt_str_concat
	mov	x1, x19
	bl	___rt_str_concat
	ldr	x19, [x29, 24]
	ldp	x29, x30, [sp], 32
	ret
/* end function cli_help_option_row */

.text
.balign 4
_cli_help_header:
	hint	#34
	stp	x29, x30, [sp, -16]!
	mov	x29, sp
	bl	_utilities_colors_orange
	bl	_utilities_colors_bold
	ldp	x29, x30, [sp], 16
	ret
/* end function cli_help_header */

.text
.balign 4
_cli_help_pad_right:
	hint	#34
	stp	x29, x30, [sp, -32]!
	mov	x29, sp
	str	x19, [x29, 24]
	mov	x19, x0
	bl	_utilities_colors_visible_length
	mov	x2, x0
	mov	x0, x19
	mov	x1, #16
	sub	x1, x1, x2
	cmp	x1, #0
	cset	x2, gt
	cmp	x2, #1
	beq	L37
	adrp	x1, _str441@page
	add	x1, x1, _str441@pageoff
	bl	___rt_str_concat
	b	L38
L37:
	mov	x19, x0
	adrp	x0, _str438@page
	add	x0, x0, _str438@pageoff
	bl	_donna_string_repeat
	mov	x1, x0
	mov	x0, x19
	bl	___rt_str_concat
L38:
	ldr	x19, [x29, 24]
	ldp	x29, x30, [sp], 32
	ret
/* end function cli_help_pad_right */

.text
.balign 4
___rt_str_concat:
	hint	#34
	stp	x29, x30, [sp, -48]!
	mov	x29, sp
	str	x19, [x29, 40]
	str	x20, [x29, 32]
	str	x21, [x29, 24]
	mov	x20, x1
	mov	x21, x0
	mov	x0, x21
	bl	_strlen
	mov	x19, x0
	mov	x0, x20
	bl	_strlen
	mov	x1, x21
	add	x0, x19, x0
	mov	x19, x1
	mov	x1, #1
	add	x0, x0, x1
	bl	_malloc
	mov	x1, x19
	mov	x19, x0
	bl	_strcpy
	mov	x1, x20
	mov	x0, x19
	mov	x19, x0
	bl	_strcat
	mov	x0, x19
	ldr	x19, [x29, 40]
	ldr	x20, [x29, 32]
	ldr	x21, [x29, 24]
	ldp	x29, x30, [sp], 48
	ret
/* end function __rt_str_concat */

